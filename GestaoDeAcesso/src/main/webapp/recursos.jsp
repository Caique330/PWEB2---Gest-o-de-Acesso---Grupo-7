...
<head>
    <meta charset="UTF-8">
    <title>Gestão de Recursos - TechMaster</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>Gestão de Recursos (Sistemas/Serviços)</h2>
            <a href="${pageContext.request.contextPath}/recursos?action=new" class="btn btn-primary">Novo Recurso</a>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <c:forEach var="rec" items="${listaRecursos}">
                    <tr>
                        <td>${rec.id}</td>
                        <td>${rec.nomeRecurso}</td>
                        <td>${rec.tipoRecurso}</td>
                        <td class="actions">
                            <a href="${pageContext.request.contextPath}/recursos?action=edit&id=${rec.id}" class="btn btn-secondary">Editar</a>
                            <c:if test="${rec.id != 1}">
                                <a href="${pageContext.request.contextPath}/recursos?action=delete&id=${rec.id}" 
                                   class="btn btn-danger" 
                                   onclick="return confirm('Tem certeza que deseja excluir ${rec.nomeRecurso}?');">
                                   Excluir
                                </a>
                            </c:if>
                        </td>
