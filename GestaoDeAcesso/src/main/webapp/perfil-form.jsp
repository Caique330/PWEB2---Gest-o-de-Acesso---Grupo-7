...
<head>
    <meta charset="UTF-8">
    <title>Formulário de Perfil - TechMaster</title>
    <style>
        ...
        .btn { text-decoration: none; padding: 0.6rem 1.2rem; border-radius: 5px; font-weight: 500; transition: background-color 0.3s ease; cursor: pointer; border: none; font-size: 1rem;}
...
<body>
    ...
    <div class="container">
        <h2>
            <c:if test="${empty perfil}">Novo Perfil</c:if>
            <c:if test="${not empty perfil}">Editar Perfil (ID: ${perfil.id})</c:if>
        </h2>

        <form action="${pageContext.request.contextPath}/perfis" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nome" name="nome" value="${perfil.nomePerfil}" required>
            </div>
            
            <div class="form-group">
                <label for="descricao">Descrição</label>
                <textarea id="descricao" name="descricao">${perfil.descricao}</textarea>
            </div>

            <div class="form-actions">
                <button type="submit" class="btn btn-primary">Salvar</button>
                <a href="${pageContext.request.contextPath}/perfis" class="btn btn-secondary">Cancelar</a>
            </div>
        </form>
    </div>
</body>
